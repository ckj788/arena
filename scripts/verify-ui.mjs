...
try {
  ...
  if (publicUrl) {
    ...
    if (joinButton) {
      ...
      await page.waitForFunction(() => document.querySelector('[aria-labelledby="join-arena-title"]')?.textContent.includes('queue temporarily unavailable'));
      await page.keyboard.press('Escape');
      results.push('Console: failed enqueue remains recoverable and does not claim success');
    }
    await clickText('Return to Arena ➔');
    await page.waitForSelector('[data-main-page="arena"] #arena-section');
    ...
    await page.click('a[href="/champions"]');
    await page.waitForSelector('[data-main-page="champions"] #champions-section');
    const anchorLength = await page.evaluate(() => history.length);
    await page.click('a[href="/champions"]');
    assert.equal(await page.evaluate(() => history.length), anchorLength, 'Repeated navigation does not duplicate history');
    await page.goBack({ waitUntil: 'domcontentloaded' });
    await page.waitForSelector('[data-main-page="arena"] #arena-section');
    results.push('History: Return to Arena and independent pages support Back/Forward without duplicate entries');
    await page.click('a[aria-label="Indie Clash home"]');
    await page.waitForSelector('#launches-section');
    assert(await page.$eval('#launches-section', (el) => getComputedStyle(el).opacity === '1'));
    results.push('Simulated signed-in console: returning to Discover preserves discovery/feed');
    ...
    await page.waitForFunction(() => location.pathname === '/arena' && !!document.querySelector('#arena-section') && !document.querySelector('.maker-console'));
