...
export default function ProductGallery({ images, title }: { images: string[]; title: string }) {
  ...
  return <section aria-label={`${title} product images`} className="mb-6" onKeyDown={e => { if (e.key === "ArrowLeft" || e.key === "ArrowRight") { e.preventDefault(); step(e.key === "ArrowLeft" ? -1 : 1); } }}>
    <button type="button" onClick={() => setExpanded(true)} aria-label="Enlarge product image" className="block w-full overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50 shadow-2xs focus-visible:outline-2 focus-visible:outline-violet-400">
    ...
    {images.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-2">{images.map((src, i) => <button key={`${src}-${i}`} type="button" onClick={() => setActive(i)} aria-label={`View image ${i + 1}`} aria-pressed={active === i} className={`w-24 shrink-0 overflow-hidden rounded-lg border-2 transition-colors ${active === i ? "border-violet-500 shadow-2xs" : "border-zinc-200 hover:border-zinc-300"}`}><img src={src} alt="" loading="lazy" className="aspect-video w-full bg-zinc-50 object-contain" /></button>)}</div>}
